...
from CASA-NER import models
from CASA-NER import sampling
from CASA-NER import util
from CASA-NER.entities import Dataset
from CASA-NER.evaluator import Evaluator
from CASA-NER.input_reader import JsonInputReader, BaseInputReader
from CASA-NER.loss import promptnerLoss, Loss
from tqdm import tqdm
from CASA-NER.trainer import BaseTrainer
...
class promptnerTrainer(BaseTrainer):
    ...
    def _train_epoch(self, model: torch.nn.Module, compute_loss: Loss, optimizer: Optimizer, dataset,
        ...
        for batch in tqdm(data_loader, total=total, desc='Train epoch %s' % epoch):
            ...
            batch_loss = compute_loss.compute(entity_logits, p_left, p_right, output, gt_types=batch['gt_types'], 
                                              gt_spans=batch['gt_spans'], entity_masks=batch['entity_masks'], 
                                              epoch=epoch, deeply_weight=args.deeply_weight, 
                                              seq_logits=masked_seq_logits, gt_seq_labels=batch['gt_seq_labels'], 
                                              batch=batch)
                        # log the memory usage after each batch
            if iteration % args.train_log_iter == 0 and self.local_rank < 1:
                allocated_memory = torch.cuda.memory_allocated(self._device) / 1024 / 1024  # in MB
                reserved_memory = torch.cuda.memory_reserved(self._device) / 1024 / 1024  # in MB
                self._logger.info(f"Memory allocated: {allocated_memory:.2f} MB")
                self._logger.info(f"Memory reserved: {reserved_memory:.2f} MB")
